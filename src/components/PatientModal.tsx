...
const PatientModal: React.FC<PatientModalProps> = ({ patient, open, onClose }) => {
  ...
  const handleEditSave = async (updatedPatientData: any) => {
    try {
      ...
      if (!storedPatients) {
        throw new Error("No patients found in storage");
      }
      
      const patients = JSON.parse(storedPatients);
      
      // Remove the old patient from the array
      const updatedPatients = patients.filter((p: Patient) => p.id !== patient.id);
      ...
      if (!storedSummary) {
        throw new Error("No patient summary found in storage");
      }
      
      const summary = JSON.parse(storedSummary);
      
      // Remove the patient from their old triage level
      summary[patient.triageLevel].patients = summary[patient.triageLevel].patients.filter(
        (p: Patient) => p.id !== patient.id
      );
      summary[patient.triageLevel].count--;
      
      // Add the patient to their new triage level
      summary[updatedPatient.triageLevel].patients.push(updatedPatient);
      summary[updatedPatient.triageLevel].count++;
